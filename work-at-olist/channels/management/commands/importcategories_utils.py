...
def fill_database(categories_list, channel_obj):
    for item in categories_list:
        if len(item)==1:
            category = Categories.objects.create(name=item[-1].strip(), channel=channel_obj)
            ...
            if len(ancestors_list) >= len(item):
                ...
                for remove in range(non_ancestors_total):
                    ancestors_list.pop()
            category = Categories.objects.create(name=item[-1].strip(), channel=channel_obj, parent=ancestors_list[-1])
